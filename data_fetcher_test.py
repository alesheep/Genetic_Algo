...
start_date = datetime(2023, 7, 8, tzinfo=ZoneInfo('UTC'))
end_date = datetime(2024, 6, 17, tzinfo=ZoneInfo('UTC'))
...
def save_data_to_csv(data_df, ticker):
    ...
    filename = f"{ticker.replace('/', '_')}_crypto_bars_data_test.csv"
    # Save the dataframe to CSV in the 'DATA' directory
    data_df.to_csv(os.path.join(data_dir, filename), index=True)
    print(f"Test data saved to {os.path.join(data_dir, filename)}")

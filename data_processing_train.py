...
def save_close_prices_to_csv(data_df, ticker):
    try:
        ...
        filename = f"{ticker.replace('/', '_')}_close_prices.csv"
        file_path = os.path.join(data_dir, filename)

        # Save the close prices to CSV in the 'DATA' directory
        close_prices.to_csv(file_path, index=False)
        print(f"Close prices saved to {file_path}")

    except Exception as e:
        print(f"Error saving close prices for {ticker}: {e}")

def process_and_save_close_prices():
    data_dir = os.path.join(os.path.dirname(__file__), 'DATA')
    for ticker in tickers:
        file_path = os.path.join(data_dir, f"{ticker.replace('/', '_')}_crypto_bars_data.csv")
    ...
    for ticker in tickers:
        ...
        if os.path.exists(file_path):
            ...
            if x_train is not None:
                print(f"Data for {ticker} prepared for LSTM model.")
            else:
                print(f"Failed to prepare data for {ticker}.")
        else:
            print(f"Close prices file for {ticker} not found: {file_path}")
